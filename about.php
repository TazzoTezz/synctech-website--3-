<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <title>About Us | Sync Tech</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta http-equiv="Content-Security-Policy" content="default-src 'self'; style-src 'self' https://fonts.googleapis.com 'unsafe-inline'; font-src https://fonts.gstatic.com; img-src 'self' data: https:; script-src 'self'; connect-src 'self';">
  <meta http-equiv="Referrer-Policy" content="strict-origin-when-cross-origin">
  <meta http-equiv="X-Content-Type-Options" content="nosniff">
  <meta http-equiv="X-Frame-Options" content="SAMEORIGIN">
  <meta http-equiv="Permissions-Policy" content="geolocation=(), microphone=(), camera=()">
  <link rel="stylesheet" href="css/style.css" />
  <link rel="icon" href="images/favicon.ico" type="image/x-icon">
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;500;700&display=swap" rel="stylesheet" crossorigin="anonymous">
</head>
<body>
  <header class="header">
    <div class="container">
      <div class="logo">
        <img src="images/logo.png" alt="Sync Tech Logo" style="height:90px;">
      </div>
      <div class="nav-toggle"><span></span><span></span><span></span></div>
      <nav class="nav">
        <a href="index.php">Home</a>
        <a href="services.php">Services</a>
        <a href="about.php" class="active">About</a>
        <a href="book.php">Book</a>
        <a href="plans.php">Plans</a>
        <a href="testimonials.php">Testimonials</a>
        <a href="contact.php">Contact</a>
        <a href="https://helpdesk.synctech.co.nz" target="_blank" rel="noopener" class="btn-black" tabindex="0" style="margin-left:10px; font-size:1.01rem; padding:8px 18px;">Helpdesk Login</a>
      </nav>
    </div>
  </header>

  <main class="container" style="padding: 60px 20px;">
    <h2 style="text-align: center;">Our Story</h2>
    <p style="max-width: 750px; margin: 0 auto 40px; text-align: center;">
      Sync Tech was founded with one goal: to make tech support easier, more approachable, and available when and where people need it. With over 13 years in the IT industry, we've helped hundreds of clients fix frustrating tech issues — and we come to you.
    </p>
    <div class="feature-list">
      <div class="feature">
        <h3>Our Mission</h3>
        <p>We're here to deliver fast, affordable, and honest tech support — mobile or remote. No complicated jargon, no surprises. Just results.</p>
      </div>
      <div class="feature">
        <h3>Who We Serve</h3>
        <p>Home users, small businesses, elderly customers, and busy professionals who just want tech to work.</p>
      </div>
      <div class="feature">
        <h3>Why It Works</h3>
        <p>We listen, explain things clearly, and focus on what matters: fixing problems quickly and affordably — no fix, no fee.</p>
      </div>
    </div>
  </main>

  <footer class="footer">
    <div class="container">
      <p>&copy; 2025 Sync Tech. Making Tech Easy.</p>
    </div>
  </footer>

  <script src="js/script.js" defer></script>
</body>
</html>